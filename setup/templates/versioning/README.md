...
4. **Declare it to the gate:** create `tools/versioning.config.json` (see
   ...
   you copied in step 1. `check-versioning` then confirms the source is **real**, not
   just named. Set a model to `null` for a conscious N-A.
...
- **Checks:** a declared `apps.model` has a resolvable `versionSource`; a declared
  ...
  declared. `null` = conscious N-A (reported, never a failure). No config → skipped.
- **Does NOT:** version your tools/scripts — that is a different artifact type
  ...
  release tooling; it verifies the wiring is present so the release can't be a no-op.
