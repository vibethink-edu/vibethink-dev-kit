...
>
> **Naming de proyecto — el nombre es CANÓNICO, no ad-hoc (regla, no detalle)** *(SEALED 2026-06-30 by the Principal Architect)*. La memoria se particiona por proyecto, y el nombre **DEBE** ser el **canónico = el nombre del git-remote del repo consumidor**. Engram lo **detecta solo del cwd** (vía git_remote) — así que estando en el repo, **no hace falta `--project`**; si lo forzás, usá el canónico, **NUNCA un alias corto/apodo**. Un nombre ad-hoc (un nick, una abreviatura) **fragmenta** la memoria en varios proyectos y **arriesga mezcla cross-sistema** — es un **finding**, no un detalle menor. `engram doctor` lo grita como `session_project_directory_mismatch` (la BD es global: un store, varios sistemas separados por proyecto — un nombre mal puesto los acerca).
>
> ⚠️ **`engram projects consolidate` es SUPERVISADO, jamás a ciegas.** Su heurística agrupa nombres "similares" **por directorio compartido**: parado en el dir de un sistema, ofrecerá fusionar memorias de **otro** sistema dentro del tuyo (contaminación). Corré el consolidate **desde el dir de cada sistema** y seleccioná **SOLO su match** — nunca `--all` ni `'all'`. `--dry-run` / `'none'` previsualiza sin mutar. *(Evidencia 2026-06-30: 4 proyectos fragmentados — un nombre corto con el grueso de las memorias, y desde un dir la auto-fusión proponía meter otro sistema adentro; el fix fue renombrar cada uno a su canónico, por separado, desde su propio dir.)*
>
