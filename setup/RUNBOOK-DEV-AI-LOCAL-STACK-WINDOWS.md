...
- **⚠️ `wsl --shutdown` corta los contenedores Docker EN SECO → Open WebUI puede quedar sin arrancar (2026-07-16).** Aplicar un cambio de `.wslconfig` (ej. el cap `processors=`) requiere `wsl --shutdown`, que mata Docker Desktop y sus contenedores abruptamente. Resultado observado: Open WebUI se queda en `uvicorn "Waiting for application startup"` sin completar (cont:8080 y host:3111 dan 000), y **cicla** cada ~4-5 min. Se descartó uno por uno: no es el volumen (pasa con volumen fresco), ni el port-forward, ni Ollama (alcanzable 200), ni HuggingFace (offline no lo arregló), ni reiniciar Docker Desktop (dio un 200 fugaz y volvió a colgar). **Queda como issue abierto** (probable: pinnear versión estable de la imagen en vez de `:main`, o revisar el lifespan). **Prevención:** `docker stop $(docker ps -q)` **antes** del `wsl --shutdown`; `docker start` después. **El runner CI NO se afecta** (systemd en WSL, no Docker → vuelve solo). Acceso remoto que sí quedó: **Tailscale Serve** (`tailscale serve --bg --http=80 3111` → `http://<nodo>` en la tailnet, sin abrir firewall; Docker publica en `127.0.0.1` y Serve lo alcanza local).
