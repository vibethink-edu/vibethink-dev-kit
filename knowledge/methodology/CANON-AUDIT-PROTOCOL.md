...
- **This is the operational mirror of the operator-tool degradation rule** (`AGENTS_UNIVERSAL` — an inherited/provisioned tool's absence *degrades, does not fail product correctness, but must not be silent*), **extended from operator tools to any inherited dependency a gate consults — including the kit mount itself.** Absent = WARN (loud, not silent, not blocking); present-but-broken = RED.
