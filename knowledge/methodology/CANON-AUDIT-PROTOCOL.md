...
> **Status:** SEALED 2026-06-05 by the Principal Architect — agnostic-lift seal sweep ("SEAL DALE"). Fire-test passed. **§8 (Verifier integrity) added + SEALED 2026-06-13 by the Principal Architect.** **§9 (Verbatim persistence of the audit record) + §10 (Completeness ladder A0–A3) added 2026-07-02 on the chief architect's GO ("GO por supuesto"), sealed on merge of their PR — from a repeated chat-delivered-review incident and the completeness question raised the same day.** **§8.8 (the mirror false-RED — a gate must not block clean work by mislabeling an absent inherited dependency as a content defect; degrade to WARN on genuine absence, security controls fail closed, resolve portably) — SEALED 2026-07-06 by the Principal Architect after one independent adversarial round (REQUEST-CHANGES: fail-closed security carve-out + falsifiability + partial-presence → APPROVE); from a consumer's inherited-mount-wiped incident that hard-blocked every docs push (D-061).**
...
### §8.8 — The mirror false-RED: a gate must not block clean work by mislabeling an absent inherited dependency as a content defect *(SEALED 2026-07-06 by the Principal Architect)*

§8's root is a verifier that lies GREEN. This is the **same lie in the other direction**: a gate that reads **RED and blocks** not because the content it guards is broken, but because an **environment the gate needs is absent** — an inherited mount, a sibling checkout, or a provisioned tool that is simply not present on this machine/worktree. Blocking clean work while *naming an environmental absence as a content-contract failure* manufactures false **friction** the way §8's green manufactures false confidence — and it trains the operator to `--no-verify`, which then bypasses the gate on the runs where the content *is* broken.

> **A gate that depends on an inherited/provisioned dependency MUST distinguish two conditions and MUST NOT conflate them: (a) the dependency is PRESENT but its content/contract is broken → RED, block (a real defect); (b) the dependency is GENUINELY ABSENT (not provisioned here) → WARN/advisory, surfaced but not a hard merge/push block — *for a quality/consistency gate; a security control fails closed instead (carve-out below).* And it MUST resolve the dependency PORTABLY — via a declared env/config, never a hardcoded machine-specific path.**

- **Verify content, not the container (ties to §8.1).** A resolver that checks *"the directory exists"* rather than *"the required content is present"* treats an **emptied** mount as present and RED-s on missing content — the worst of both: it neither degrades (it blocks) nor classifies honestly (it blames the content). Resolve to the dependency, then check the dependency's *content*.
- **A hardcoded path is not resolution.** A machine-specific fallback path makes the gate machine-bound and turns a portable absence into a phantom-present (the path "exists" empty). Portable resolution (env var / declared config, with a documented order) is §8.1's "classify by the observed condition" applied to *where the dependency is*, not a guess.
- **This is the operational mirror of the operator-tool degradation rule** (`AGENTS_UNIVERSAL` — an inherited/provisioned tool's absence *degrades, does not fail product correctness, but must not be silent*), **extended from operator tools to any inherited dependency a gate consults — including the kit mount itself.** Absent = WARN (loud, not silent, not blocking); present-but-broken = RED. *(That original rule is safe because its objects are by definition inert — "NEVER a product-correctness dependency". Extending it to a class that can include security-relevant dependencies — the policy engine's own guards live in the kit mount — requires carrying the risk limit with it: the carve-out below.)*
...
- **The absence branch is itself subject to §8.7(a).** The (a)/(b) classifier and the WARN verdict are provable behavior: the gate's paired test ships a **known-absent fixture that must WARN (not RED, not block — for a quality/consistency gate; a security control fails closed per the carve-out above)** and a **known-present-but-broken fixture that must RED**. An unproven classifier that drifts toward "absent" turns every real breakage into ambient WARN — §8's false-green, one level up.
...
**Precedent.** On a machine where the inherited kit mount had been wiped to an **empty directory**, a consumer's knowledge-baseline pre-push gate **hard-blocked every docs/comms/specs/features push** — it resolved the mount by a hardcoded machine path that still "existed" (empty), then RED-ed on the (now-missing) required files, forcing a `--no-verify` on a **clean docs PR**. The content was fine; the *environment* was absent, and the gate could not tell the difference. Restoring the mount fixed the instance; the class is fixed by this rule. The L3 incarnation (the wired gate, its env-based resolution, its WARN-on-absent behavior) lives in the consuming repo.
