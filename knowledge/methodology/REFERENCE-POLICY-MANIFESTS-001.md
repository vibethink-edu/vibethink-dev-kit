...
| `watch` | the instrument that watches the rule: `{kind:"gate", ref}` (a check-\* engine) · `{kind:"golden-task", ref, task}` (a battery trap) · `{kind:"none", note}` — **unwatched is a conscious declaration with a reason, never an omission** |
...
- **The gate makes divergence visible** (`check-policy-manifests.mjs`, on the doctor board and in CI): shape · source exists + id in title-or-filename · **status parity** with the prose · § anchor per rule · watch refs exist (golden-task watches must name a real battery task) · coverage ratchet.
...
- `requireFor` in `tools/policy-manifests.config.json` is a **ratchet**: a canon listed there MUST have a manifest, and the list only grows — never shrinks.
