...
reported for every pair and both orderings, **over the co-provisioned population only** (§5 — a deliberately-sparse source's absences are not drift). Empty in all directions = the sources agree. Non-empty = named drift, each entry a lockout-or-ghost candidate.

**Third class — attribute divergence (not presence/absence).** Presence/absence is only two of the three drift classes. A principal **active in both sources but carrying a different authority level in each** (top-privilege in one, a lesser role in the other) is neither a lockout (present everywhere) nor a ghost (active everywhere) — it is a **silent wrong-privilege**, and a set-difference alone declares "the sources agree" and misses it. **Where both sources carry the same authority attribute for a principal, the check MUST also compare that attribute** and report a divergence as a third drift entry. (A source that does not carry the attribute at all cannot be compared on it — that pair is presence/absence only.) This closes the gap for a canon whose §7 claims to govern *the relationship between sources*, not merely co-existence.
...
- **The co-provisioned population — the scoping that makes the drift-check correct.** Drift is measured only over the principals the system *intends* to have in **all** the sources under check — those it co-provisions via §2.1. A source can be **deliberately sparse**: one where a principal's **absence is itself the authorization decision**, not an incomplete provisioning. Two common shapes, both in-scope for the canon but **not** drift on the sparse direction: **(a) fail-closed linking** — an authenticated principal with *no* row in a second store (the store that answers "what may they do") is a *normal, correct denial*, not a locked-out legitimate principal; **(b) JIT provisioning** — a federated IdP where the local row is created at first use, so "active in the IdP, no local row yet" is the expected pre-first-use state. A deliberately-sparse source (and the direction that reads it) **MUST be declared** in the L3 binding (§6) and is **excluded from the drift-check on that direction** — the check still runs on the bounded direction (principals the system *did* co-provision but that diverge). Without this scoping, the check reports a system's own correct fail-closed design as a mass "security fault." *(§1's "present in one and absent in another = drift" is always read through this scope: absence in a **deliberately-sparse** source is by-design, not drift.)*
