...
- **Consumer kit mount = isolated, link-free clone, never a junction/reparse-point — PREVENT the follow-delete class, don't only mitigate it (Amendment 2026-07-13, D-066):** the three rules above *mitigate* the delete (never follow the link). The **prevention** is to not create the link at all. A consuming repo's inherited kit checkout (the sibling the repo's scripts resolve) **MUST be an isolated clone — its own `.git`, reached WITHOUT a junction/symlink/reparse-point anywhere in the path** — never a link to a shared kit checkout. Rationale: a clone is *structurally immune* to junction-follow-delete — a recursive delete of the clone can only destroy the clone (re-clonable in seconds) and **can never escape into the shared/canonical kit**; a link can. (A single shared sibling clone that several repos reference *by path* satisfies "link-free + own `.git`", but its blast radius is wider — one wipe blocks **every** consumer — so a per-consumer clone is preferable where disk allows.) The junction's one benefit — instant currency with the source — is **traded** for the consumer's own `devkit-upgrade --ff-only` pull: this exchanges a rare catastrophic wipe for **ordinary staleness** (a clone can drift if the pull is skipped), which is **non-destructive and healed by the next pull's canon-delta** — the correct trade, but the residual staleness is the consumer's to manage (a freshness signal is a separate concern, not assumed here). **Evidence — one *confirmed* junction-follow-delete** *nearly took the canonical kit* (D-062, the same-day double-destruction); that alone justifies the rule. Two further mount-empty events (D-061's false-RED mount; a consumer mount emptied to 0 items, 2026-07-13) are the **related mount-broken class** — a clone does **not** stop an in-place delete from emptying it, but it **stops the damage from escaping to the shared kit and makes recovery a fast re-clone** (the 2026-07-13 restore *as an isolated clone* is the proof). **Classification (a non-blocking health-board signal — the empty case is the `CANON-AUDIT-PROTOCOL §8.8` *absent-class* → WARN, never a hard block on quality gates):** `devkit-doctor`'s `mount-integrity` check classifies the inherited kit **as the consumer reaches it** — via the *invoked* path, since Node realpath-resolves `import.meta.url` and a check on the resolved root cannot see the link. A **link anywhere in the reach path, or a consumer convenience link (e.g. an in-repo mount) → WARN** (wipe-risk; use a clone); an **empty/non-`git` mount → WARN, alongside the universal-rulebook error** (present-but-broken is surfaced, never silently green); a **link-free own-`.git` kit → OK**. The clone-setup executor (the concrete `git clone` + path) stays **L3** (OS/path-specific); the kit prescribes the *rule* + the *classification*. This is the setup-side complement to D-062's delete-side rules — prevention + mitigation together close the mount-escape class.
