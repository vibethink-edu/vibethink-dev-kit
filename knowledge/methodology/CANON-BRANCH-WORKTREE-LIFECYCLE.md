...
- **Consumer kit mount = isolated clone, never a junction/reparse-point — PREVENT the follow-delete class, don't only mitigate it (Amendment 2026-07-13, D-066):** the three rules above *mitigate* the delete (never follow the link). The **prevention** is to not create the link at all. A consuming repo's inherited kit checkout (the sibling/mount the repo's scripts resolve, e.g. `../vibethink-dev-kit`) **MUST be an isolated clone** — its own `.git`, cloned from the kit remote — **never a junction / symlink / reparse-point to a shared kit checkout**. Rationale: a clone is *structurally immune* to junction-follow-delete — a recursive delete of the clone destroys only the clone (re-clonable in seconds), and **can never reach the shared/canonical kit**; a junction can. The junction's one benefit (instant currency with the source) is already replaced by the governed `devkit-upgrade` pull (`--ff-only` + canon-delta) the consumer runs anyway. **Evidence: the inherited mount was wiped to an empty/broken directory three times** — the D-061 false-RED mount, the D-062 same-day double-destruction (one wipe *nearly took the canonical kit*), and a consumer's mount emptied to 0 items (2026-07-13); each blocked **all** inherited gates until restored. The 2026-07-13 restore **as an isolated clone** broke the cycle. **Detection (mechanizes `CANON-AUDIT-PROTOCOL §8.8` for the mount):** a gate that resolves the inherited kit MUST classify it — a **reparse-point/junction mount → WARN** (the wipe-risk pattern; migrate to an isolated clone), an **empty or non-`git` mount → RED/block** (present-but-broken, never fail-open on inherited gates), a **healthy clone → OK**; `devkit-doctor` carries this classification. The clone-setup executor (the concrete `git clone` invocation + path) stays **L3** (OS/path-specific); the kit prescribes the *rule* + the *detection*. This is the setup-side complement to D-062's delete-side rules — together, prevention + mitigation close the mount-destruction class.
