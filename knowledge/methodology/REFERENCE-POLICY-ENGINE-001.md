...
- **Item 6 (consumption lens) — SHIPPED (2026-07-02 addendum, closes S3-P4):** `policy-engine report` (`tools/policy-engine/telemetry-lens.mjs` — pure functions; the CLI does the I/O) reads the emitted JSONL (default: the hook adapter's session dir; `--telemetry` repeatable) and reports **verdict mix + emission window/freshness** (is the wire alive?), **DENY/ASK ranking by deciding rule** (which law generates friction), **friction streaks** (≥ `--streak` consecutive DENYs, same session+rule — over-match suspect or a flow needing a governed grant; lengths annotated as upper bounds when the log carries malformed lines, since a corrupt record may have been a run-breaker), and — when manifests resolve (optional here) — **never-fired enforceable rules** (deterrence or dead pattern; prove which with a known-bad, §8.7a). Advisory read-only lens, never a gate and never a second source of verdicts: exit 0 with or without data; corrupt lines are counted, not fatal; exit 2 only for an explicitly named source that cannot be read. The numbers feed manifest tuning (warn→block, grant shapes, pattern fixes) — the "real data to tune warn→block" the S3 row promised.
