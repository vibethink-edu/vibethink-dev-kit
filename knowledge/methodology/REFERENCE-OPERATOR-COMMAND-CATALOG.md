...
8. **One canonical trigger per intention.** The catalog defines exactly **one** canonical trigger per command id. Synonym aliases are **not** shipped as canon — field use showed they bloat the help list, breed dead/duplicate entries, and let a short synonym fire before the long one (rule 6). An alias is **opt-in and local (L3)**: a consumer may add one to their own adapter for a **documented reason** (a genuine typo-magnet, a localized daily-use short form), prefix-safe (rule 6), never overriding or duplicating the canonical. The catalog lists one trigger; `commands-help` lists exactly the canonical set. *(Supersedes the former "short trigger + long synonym" convention; validated by the §5.1 gate.)*
9. **Declared execution.** A command's output **opens by declaring which command ran and its intent** — `COMANDO EJECUTADO: :xxx — <intención>` (localized label allowed; rule 5) — so the transcript records what the operator triggered and the agent cannot silently drift from it. A purely static listing that executes nothing (e.g. `commands-help`) declares in its **header line**, not via the evidence preamble. *(Reinforces rule 2's evidence discipline.)*
