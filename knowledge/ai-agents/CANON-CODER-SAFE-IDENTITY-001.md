...
> **Platform note — enforcement is host-dependent** *(amendment SEALED 2026-06-18 — first-heir macOS finding; D-022)*. The lock is a *hard* lock only where the host **enforces** branch protection. On some hosted plans / repo visibilities, enforcement is unavailable or gated behind a **paid org tier** (e.g. on a major host, branch-protection **rulesets on a *private* repo are not enforced** without a paid org plan — the host warns the ruleset "won't be enforced"). Where the host cannot enforce it, the lock degrades to **discipline, not enforcement**. So: **confirm the host actually enforces the protection** before relying on it; if it cannot, the L3 binding (§10) MUST declare the fallback — a **public** repo, an **alternative enforcement that the host's permission model actually supports**, or an explicit **discipline-only `N-A(reason)`**. **The fallback is itself host-dependent — confirm it before relying on it.** A permission-based fallback (the bot has no merge rights on the target — e.g. a fork model where the bot is read-only on the target and works on its own fork) only holds where the host allows that permission shape; on some hosts a **read-only collaborator exists only under an org account, not a personal one** (so the fork hard-lock needs an org — a free org suffices — not a personal repo). The identity asymmetry (bot = propose-only) still holds as a rule; what varies is whether the platform *enforces* it or it rides on discipline.
